...
const StockEntryDetailPage: React.FC = () => {
  ...
  const isStockEntryActive = result?.status === "ACTIVE" ?? false;
    ...
    return editStockMovements(
      {
        ...
        status: isStockEntryActive ? "INACTIVE" : "ACTIVE",
  ...
  const { isPending, mutate } = useMutation({
    ...
    onSuccess: () => {
      message.success(
        `Entrada ${
          isStockEntryActive ? "Desactivada" : "Activado"
        } correctamente`
      );
    ...
    onError: () => {
      message.error(
        `Error al ${isStockEntryActive ? "Desactivar" : "Activar"}  la entrada`
      );
    },
  });
            ...
            <div className="flex gap-2">
              ...
              {result && (
                ...
                >
                  ...
                  >
                    {isStockEntryActive ? "Desactivar" : "Activar"}
                  </Button>
                </Popconfirm>
              )}
            </div>

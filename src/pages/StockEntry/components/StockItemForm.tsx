...
import { StockEntryItems, entryItemSchema } from "../stockEntrySchema";
...
interface Props {
  ...
  setItems: React.Dispatch<React.SetStateAction<StockEntryItems[]>>;
}

type entryItemPayloadType = z.infer<typeof entryItemSchema>;

const StockItemForm: React.FC<Props> = ({ setItems, items }) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
  ...
  const handleAddItems = (newItem: StockEntryItems) => {
    ...
    if (existingItem) {
      const updatedItems = items.map((item: StockEntryItems) => {
        if (
          item.productId === newItem.productId &&
          item.batch === newItem.batch &&
          item.price === newItem.price

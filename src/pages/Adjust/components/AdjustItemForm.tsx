...
import { AdjustItems, adjustItemSchema } from "../adjustSchema";

interface Props {
  items: AdjustItems[];
  setItems: React.Dispatch<React.SetStateAction<AdjustItems[]>>;
}

type adjustItemPayloadType = z.infer<typeof adjustItemSchema>;

const AdjustItemForm: React.FC<Props> = ({ setItems, items }) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
    ...
    reset,
    watch,
  } = useForm<adjustItemPayloadType>({
    resolver: zodResolver(adjustItemSchema),
  });
  const [selectedProduct, setSelectedProduct] = React.useState<
    Product | undefined
  >(undefined);

  const handleAddItems = (newItem: AdjustItems) => {
    ...
    if (existingItem) {
      const updatedItems = items.map((item: AdjustItems) => {
        if (
          item.productId === newItem.productId &&
          item.batch === newItem.batch &&
          item.price === newItem.price
